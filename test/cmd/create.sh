...
run_create_job_tests() {
    ...
    kubectl create job test-job --image=harbor.ultra.com/k8s/nginx:test-cmd
    # Post-Condition: job nginx is created
    kube::test::get_object_assert 'job test-job' "{{$image_field0}}" 'harbor.ultra.com/k8s/nginx:test-cmd'
    # Clean up
